...
# Mi42 Onboarding Execution Script (Simplified - no jq required)
...
execute_task() {
    ...
    SYSTEM_ESCAPED=$(echo "$SYSTEM_PROMPT" | sed 's/\\/\\\\/g' | sed 's/"/\\"/g' | sed 's/$/\\n/' | tr -d '\n' | sed 's/\\n$//')
    USER_ESCAPED=$(echo "$USER_PROMPT" | sed 's/\\/\\\\/g' | sed 's/"/\\"/g' | sed 's/$/\\n/' | tr -d '\n' | sed 's/\\n$//')
    
    # Call OpenAI API
    RESPONSE=$(curl -s https://api.openai.com/v1/chat/completions \
        -H "Content-Type: application/json" \
        -H "Authorization: Bearer $OPENAI_API_KEY" \
        -d "{\"model\":\"gpt-4o-mini\",\"messages\":[{\"role\":\"system\",\"content\":\"$SYSTEM_ESCAPED\"},{\"role\":\"user\",\"content\":\"$USER_ESCAPED\"}],\"temperature\":0.7,\"max_tokens\":4000}" \
        --max-time 60)
    
    # Extract content from response (simple grep/sed approach)
    CONTENT=$(echo "$RESPONSE" | grep -o '"content":"[^"]*"' | head -1 | sed 's/"content":"//' | sed 's/"$//' | sed 's/\\n/\n/g' | sed 's/\\"/"/g')
    ...
    CONTENT_SQL=$(escape_sql "$CONTENT")
    
    # Update task with result
    docker exec mi42-db mysql -umi42_user -pmi42_password_2025 mi42_db -e \
        "UPDATE agent_tasks SET taskStatus = 'completed', completedAt = NOW(), result = '$CONTENT_SQL', creditsActual = $CREDITS WHERE id = $TASK_ID;" 2>/dev/null
    
    # Create briefing
    BRIEFING_TITLE="${AGENT_NAME}: ${USER_PROMPT:0:100}"
    BRIEFING_TITLE_SQL=$(escape_sql "$BRIEFING_TITLE")
    
    # Simple JSON without jq
    BRIEFING_DATA="{\"title\":\"$BRIEFING_TITLE\",\"agentType\":\"$AGENT_TYPE\",\"prompt\":\"$USER_PROMPT\",\"response\":\"${CONTENT:0:500}...\",\"generatedAt\":\"$(date -Iseconds)\"}"
    BRIEFING_DATA_SQL=$(escape_sql "$BRIEFING_DATA")
    
    docker exec mi42-db mysql -umi42_user -pmi42_password_2025 mi42_db -e \
        "INSERT INTO agent_briefings (userId, taskId, briefingTitle, briefingData, isOnboarding, language, createdAt) VALUES ($USER_ID, $TASK_ID, '$BRIEFING_TITLE_SQL', '$BRIEFING_DATA_SQL', 1, 'de', NOW());" 2>/dev/null
